...
from finance_workbench.analysis import FinanceAnalysisPacket, build_claim, build_scaled_benchmark
...
class FinanceWorkbenchTests(unittest.TestCase):
    def setUp(self) -> None:
        self.current = {
            "bank": "테스트저축은행","product": "정기예금","top_12m": "3.70","base_12m": "3.60",
            "reg_date": "2026-09-10 00:00:00","url": "https://example.com/product",
        }
        self.previous = {
            "bank": "테스트저축은행","product": "정기예금","top_12m": "3.50","base_12m": "3.40",
            "reg_date": "2026-09-09 00:00:00","url": "https://example.com/product",
        }

    def test_rate_evidence_preserves_source_locator(self) -> None:
        item = build_rate_evidence(self.current, term_months=12, snapshot_updated_at="2026-09-10 18:39:11")
    ...
    def test_rate_change_is_traceable_to_two_snapshots(self) -> None:
        packet = build_rate_change_packet(self.current, self.previous, term_months=12)
    ...
    def test_scaled_benchmark_matches_actual_opb_ratio(self) -> None:
        result = build_scaled_benchmark(
            simulated_opb="750.8", actual_opb="317.1",
    ...
    def test_packet_rejects_untraceable_claim(self) -> None:
        ...
        claim = build_claim("12개월 최고금리는 3.70%입니다.", evidence_ids=[evidence.evidence_id])
        packet = FinanceAnalysisPacket(title="금리 분석", as_of="2026-09-10", evidence=[evidence.to_dict()], claims=[claim])
    ...
    def test_firm_template_contracts_force_source_trace(self) -> None:
        ...
        manifest = build_output_manifest(excel, analysis_packet_id="packet-1", source_count=2)
